#!/usr/bin/env python3
"""Export human-selected AI learning examples for later training curation."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--latest-only", action="store_true", help="Keep only the newest selection per candidate/scope/reviewer.")
...
def query(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    sql = f"""
        SELECT {distinct}
            id, candidate_key, audit_scope, reviewer, reason,
            ai_review_ref, ai_review_event_id, learning_json, created_at
