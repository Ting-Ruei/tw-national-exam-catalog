...
| **選項續行被截斷** | **新發現、未修、未標記**：抽樣 2.06%（265 題／160 卷），`quality_status` 仍是 `pass` |
...
| 段 | 做什麼 | 為何是這個順序 |
|---|---|---|
| **1. 偵測** | 在 `repair.py` 為**選項本體**加上與題幹同構的折行測試 | 先讓它變成一個**被標記的** dispute，才有人能審 |
| **2. 標記** | 新的 dispute kind，例如 `option-continuation-loss`，severity `blocker` | 不標記就等於現在：pass/None |
| **3. 驗收** | 在 S6 報「兩引擎都看到、且仍未修」的欄位數 | 這是**可下降的數字**，不是一個說法 |
