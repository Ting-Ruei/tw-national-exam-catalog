...
## 6.3 題數不符：兩個缺陷，不是六份壞卷（2026-09-21）

六份 `醫師(二)` 卷長期被 S3 以 `count-mismatch` 擋下（讀成 2 / 15 / 56 / 67 / 71 題，滿分 80），
看起來像六個互不相關的病。實際上是**兩個讀取器的缺陷**，現在都修好了。
完整報告：`qbr/reports/count_mismatch_two_causes.md`。
...
| 卷 | 缺陷 A | 缺陷 B |
|---|---|---|
| `1001_醫學(三)` / `1031_醫學(三)` / `1031_醫學(六)` / `1031_醫學(四)` | ✓ |  |
| `1002_醫學(五)` / `1012_醫學(五)` | ✓ | ✓ |

**單獨修 A 不夠，單獨修 B 也不夠**（實測：A alone 改變 0 卷、B alone 改變 7 卷、
both 改變 19 卷）。一份卷可以斷在同一個地方，卻疊了兩層原因。
