...
第三圈剩下的 7 題（`111020:305 q059–q073`，藥動學計算）逐題看過紙本：**沒有抽字缺陷**。
`q061` 的 queue stem 是完整的，而 PDF 文字層那一行是**截斷的**——管線把紙本被截斷的行重建得比
PDF 文字層還完整。那些 block 是審核者對**內容**的判斷。把內容爭議當抽取缺陷去修，就會走上
charter 說的「規則 → 腳本 → 新問題 → 新規則」跑步機。
