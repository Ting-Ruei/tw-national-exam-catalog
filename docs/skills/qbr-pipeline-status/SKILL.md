...
### 第三個陷阱：`--learned` 沒有 `action="append"`，整個方向靜默變成亂碼

這一條是**第一次全庫掃描真的踩到的**，而且它以成功的外表失敗。

`--learned` 少了 `action="append"`，argparse 只留**最後一個**旗標；然後 `parse_learned` 把那個
**字串**當成序列**逐字元**走，於是每一筆紀錄存的 learned 是：

```
- ：\n- F\n- I\n- G\n- U\n- R\n- E\n- ...（單字元項目）
```

它**安靜地**錯，因為一團垃圾在提示詞裡仍然讀作「已經知道的事」——模型就把它忽略掉了。
測量：那一輪 5,803 筆 corpus finding 中，**0 筆**引用「既知形狀」（對照：正確帶入的那一輪 60 筆裡有
32 筆引用）。也就是說那一輪的「提示詞調整」根本**沒有發生**，但報表上看起來跟有發生一樣。

**還有第二個洞：`prompt_version` 當時看不到這個差別。** 它只雜湊 `LEARNED` 的**模板**，
沒有雜湊 learned 的**內容**，所以「叫它忽略 A」與「叫它忽略 B」得到同一個雜湊，
而那些亂碼紀錄也宣稱跟正確的一輪同一個版本。現在雜湊的是**渲染出來的區塊**，
`make_record` 與 `--restale` 都傳自己實際在用的那一份。

**教訓：一個「可調的提示詞」要能被證明真的調到了。** 「有傳參數」不等於「提示詞裡有那段話」，
而「提示詞裡有那段話」不等於「紀錄能分辨是哪一段話」。這兩個都要有欄位。

### 重問：`--restale`

提示詞修好之後，以前用舊提示詞問的答案就成了孤兒——紀錄是 append-only，而那則筆記是關於一個
**已經不再是現在這個問題**的問題。`--restale` 只重問「**現行** finding 屬於舊 prompt generation」的題目：

* **只看最新一筆。** 看每一筆的話，一題會因它那則舊筆記而永遠顯得過時、每次都被重問。
* **framing 分開過濾。** 正在跑的全庫掃描不會因為一個修正落地就變過時——它是刻意的不同量測；
  不按 `population` 過濾的話，`--restale` 在跑的期間會試著重問全部 79,018 題。

```bash
.venv/bin/python scripts/ask_about_blocks.py --queue data/review-queues/live --all --restale
```
